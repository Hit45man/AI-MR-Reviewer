# LiteLLM → Bedrock (IRSA for service account platform/litellm)
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          ...
          "${local.oidc_issuer_host}:sub" = "system:serviceaccount:platform:litellm"
        }
      }
    }]
  })
...
# IAM policy for AWS Load Balancer Controller (AWS documented permissions, condensed for POC)
